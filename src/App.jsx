    ...
    <div className="login-container">
      <form className="login-box" onSubmit={handleLogin}>
        ...
        <p style={{ marginTop: '15px', color: '#888', fontSize: '0.9rem' }}>
          New here? <span onClick={() => setIsRegistering(true)} style={{ color: '#6366f1', cursor: 'pointer', fontWeight: 'bold' }}>Create Account</span>
        </p>
      </form>
    </div>
  );
    ...
    <Router>
      <div className="dashboard-container">
        <nav className="sidebar">
          ...
          <NavLink to="/analytics" className={({ isActive }) => `menu-item ${isActive ? 'active' : ''}`}>Analytics</NavLink>
          <NavLink to="/history" className={({ isActive }) => `menu-item ${isActive ? 'active' : ''}`}>History</NavLink>
